...
export default function FinanceiroCAPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editando) {
        await axios.put(
          `${API_URL}/financeiro/ca/${editando}`,
  ...
  const handleExcluir = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      await axios.delete(`${API_URL}/financeiro/ca/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const gerarRelatorioMensal = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `relatorio_ca_${mes}_${ano}.pdf`);
  ...
  return (
    <div className="container mx-auto py-4 px-2 max-w-8xl">
      <h1 className="text-lg font-bold mb-3">Controle Financeiro do CA</h1>
